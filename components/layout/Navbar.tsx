      ...
      <nav className={`backdrop-blur-md border rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.15)] px-6 py-3 flex justify-between items-center transition-all duration-300 relative ${isDarkMode ? 'bg-[#111827]/90 border-white/5' : 'bg-white/90 border-slate-200'}`}>
        ...
        <div className="hidden lg:flex items-center">
          ...
          <div className="flex items-center gap-3 pl-6 border-l border-slate-700/30">
            ...
            <a href="/cv-jhonatan.pdf" download className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-bold text-xs uppercase tracking-wider transition-all shadow-lg hover:shadow-indigo-500/25 ${isDarkMode ? 'bg-white text-slate-900 hover:bg-indigo-50' : 'bg-slate-900 text-white hover:bg-slate-800'}`}>
              <Download size={16} />
              <span>CV</span>
            </a>
          </div>
      ...
      <div className={`absolute top-full left-0 w-full mt-4 p-4 rounded-2xl backdrop-blur-xl border shadow-2xl transition-all duration-300 origin-top ${isMobileMenuOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-4 pointer-events-none'} ${isDarkMode ? 'bg-[#111827]/95 border-white/10' : 'bg-white/95 border-slate-200'}`}>
        <div className="flex flex-col space-y-2">
          ...
          <div className="flex items-center justify-between p-2">
            <span className={`text-sm font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Tema</span>
            <button 
              onClick={toggleTheme}
          ...
          <a href="/cv-jhonatan.pdf" download className={`flex items-center justify-center gap-2 p-4 rounded-xl font-bold text-sm uppercase tracking-wider transition-all mt-2 ${isDarkMode ? 'bg-white text-slate-900' : 'bg-slate-900 text-white'}`}>
            <Download size={18} />
